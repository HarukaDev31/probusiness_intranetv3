...

    {
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h(UButton, {
                ...
                size: 'xs',
                onClick: () => {
                    navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/documentacion/${row.original.id_cotizacion}`)
                }
            },
            )
        }
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h('div', { class: 'flex items-center gap-1' }, [
                ...
                h(UButton, {
                    ...
                    size: 'xs',
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/documentacion/${row.original.id}`)
                    }
                })
            ])
        }
...
watch(() => tab.value, async (newVal) => {
    if (newVal && newVal !== '') {
        try {
            if (newVal === 'general') {
                navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/${id}?tab=general`)
                ...
                navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/${id}?tab=embarcados`)
                try { searchEmbarcados.value = '' } catch (e) { /* ignore */ }
                await getEmbarcados(Number(id))
            } else if (newVal === 'variacion') {
                navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/${id}?tab=variacion`)
                try { searchVariacion.value = '' } catch (e) { /* ignore */ }
                await getClientesVariacion(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/documentacion/abiertos/clientes/${id}?tab=pagos`)
