<template>
    <div class="">
        <DataTable v-if="tab === 'prospectos'" title="" icon="" :data="cotizaciones" :columns="getProespectosColumns()"
            ...
            :previous-page-url="((currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES) || currentId == ID_JEFEVENTAS || currentRole == ROLES.ADMINISTRACION) ? `/cargaconsolidada/documentacion/abiertos/pasos/${id}` : `/cargaconsolidada/documentacion/abiertos`"
            :show-body-top="true">
            <template #body-top>
        ...
        cell: ({ row }: { row: any }) => {
            return h('div', { class: 'flex gap-2' }, [
                ...
                h(UButton, {
                    ...
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/documentacion/abiertos/cotizaciones/documentacion/${row.original.id}`)
                    }
                }),
                h(UButton, {
                    icon: 'i-heroicons-trash',
            ...
            return h('div', {
                ...
                return h('div', {
                    ...
                    h(UButton, {
                        ...
                        size: 'md',
                        onClick: () => {
                            navigateTo(`/cargaconsolidada/documentacion/abiertos/cotizaciones/proveedor/documentacion/${proveedor.id}`)
                        }
                    }),
                    h(UButton, {
    ...
    if (newVal && newVal !== '') {
        try {
            ...
            if (newVal === 'prospectos') {
                navigateTo(`/cargaconsolidada/documentacion/abiertos/cotizaciones/${id}?tab=prospectos${idCotizacionQuery}`)
                ...
                navigateTo(`/cargaconsolidada/documentacion/abiertos/cotizaciones/${id}?tab=embarque${idCotizacionQuery}`)
                try { search.value = '' } catch (e) { /* ignore */ }
                await getCotizacionProveedor(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/documentacion/abiertos/cotizaciones/${id}?tab=pagos${idCotizacionQuery}`)
