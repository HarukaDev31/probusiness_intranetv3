...
const goBack = () => {
  navigateTo(`/cargaconsolidada/documentacion/completados/pasos/${id}`)
}
...
watch(activeTab, async (newVal, oldVal) => {
  ...
  if (newVal === 'general') {
    navigateTo(`/cargaconsolidada/documentacion/completados/cotizacion-final/${id}?tab=general`)
    await getGeneral(Number(id))
  }
  if (newVal === 'pagos') {
    navigateTo(`/cargaconsolidada/documentacion/completados/cotizacion-final/${id}?tab=pagos`)
    await getPagos(Number(id))
  }
  await getHeaders(Number(id))
