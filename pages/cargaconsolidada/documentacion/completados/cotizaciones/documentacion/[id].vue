        ...
        <UCard v-if="proveedorActivo" class="p-6 rounded-lg shadow-md">
          ...
          <div class="space-y-4">
            <p class="text-sm text-gray-500 dark:text-gray-400">
              ...
              <template v-else>Hasta 4 archivos por proveedor. Los cambios de cada pestaña se conservan; «Guardar todo» guarda documentos e imágenes de todos los proveedores.</template>
            </p>
            ...
            <!-- Agregar imágenes: solo si no es solo lectura -->
...
function getFileForTipoAndProveedor(tipo: string, idProveedor: number): DocItem | undefined {
  return (files.value ?? []).find(
    (f: DocItem) => f.tipo_documento === tipo && (f.id_proveedor != null && f.id_proveedor === idProveedor)
  )
}
...
function navigateBack() {
  ...
  if (idContenedor) {
    navigateTo(`/cargaconsolidada/documentacion/completados/cotizaciones/${idContenedor}`)
  } else {
    navigateTo('/cargaconsolidada/documentacion/completados')
  }
}
