    ...
    <template>
        <div class="p-0 md:p-6">
            <DataTable v-if="tab === 'general'" title="" icon="" :data="clientes" :columns="getColumnsGeneral()"
                ...
                :filters-value="filtersGeneral" :show-export="(currentId == ID_JEFEVENTAS || COTIZADORES_WITH_PRIVILEGES.includes(currentId)) ? true : false" :show-body-top="true"
                ...
                :previous-page-url="((currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES) || currentId == ID_JEFEVENTAS || currentRole === ROLES.DOCUMENTACION || currentRole === ROLES.JEFE_IMPORTACIONES || currentRole == ROLES.ADMINISTRACION) ? `/cargaconsolidada/documentacion/completados/pasos/${id}` : `/cargaconsolidada/documentacion/completados`">
                <template #body-top>
            ...
            <DataTable v-if="tab === 'variacion'" title="" icon="" :data="clientesVariacion" :columns="columnsVariacion"
                ...
                :hide-back-button="false"
                :show-pagination="false" @export="exportData"
                :previous-page-url="((currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES) || currentId == ID_JEFEVENTAS || currentRole === ROLES.DOCUMENTACION || currentRole === ROLES.JEFE_IMPORTACIONES) ? `/cargaconsolidada/documentacion/completados/pasos/${id}` : `/cargaconsolidada/documentacion/completados`"
                empty-state-message="No se encontraron registros de clientes."
            ...
            <DataTable v-if="tab === 'pagos'" title="" icon="" :data="clientesPagos" :columns="columnsPagos"
                ...
                :show-body-top="true"
                :show-pagination="false" @export="exportData"
                :previous-page-url="((currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES) || currentId == ID_JEFEVENTAS) ? `/cargaconsolidada/documentacion/completados/pasos/${id}` : `/cargaconsolidada/documentacion/completados`"
                empty-state-message="No se encontraron registros de clientes."
...

    {
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h(UButton, {
                ...
                size: 'xs',
                onClick: () => {
                    navigateTo(`/cargaconsolidada/documentacion/completados/clientes/documentacion/${row.original.id_cotizacion}`)
                }
            },
            )
        }
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h('div', { class: 'flex items-center gap-1' }, [
                ...
                h(UButton, {
                    ...
                    size: 'xs',
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/documentacion/completados/clientes/documentacion/${row.original.id}`)
                    }
                })
            ])
        }
...
watch(() => tab.value, async (newVal) => {
    if (newVal && newVal !== '') {
        try {
            if (newVal === 'general') {
                navigateTo(`/cargaconsolidada/documentacion/completados/clientes/${id}?tab=general`)
                ...
                navigateTo(`/cargaconsolidada/documentacion/completados/clientes/${id}?tab=embarcados`)
                try { searchEmbarcados.value = '' } catch (e) { /* ignore */ }
                await getEmbarcados(Number(id))
            } else if (newVal === 'variacion') {
                navigateTo(`/cargaconsolidada/documentacion/completados/clientes/${id}?tab=variacion`)
                try { searchVariacion.value = '' } catch (e) { /* ignore */ }
                await getClientesVariacion(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/documentacion/completados/clientes/${id}?tab=pagos`)
