<template>
    <div class="md:p-6">
        <PageHeader title="" subtitle="" icon="" :hide-back-button="false"
            @back="navigateTo(`/cargaconsolidada/documentacion/completados/clientes/${cliente?.id_contenedor}`)">
            <template #actions>
                <!--button save-->
        ...
        <div v-else-if="hasData" class="md:mt-6">
            ...
            <div v-if="proveedorActivo" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <UCard class="bg-white dark:bg-gray-800 md:p-6 rounded-lg shadow-md " :class="{ 'col-span-2': (currentRole === ROLES.COORDINACION || currentRole === ROLES.JEFE_IMPORTACIONES) }">
                    <template #header>
                        <div class="flex items-center justify-between flex-col md:flex-row gap-2 md:gap-0">
                            <div class="flex items-center gap-2">
...
const isDocumentacion = computed(() => {
    ...
    return role === ROLES.DOCUMENTACION || role === ROLES.JEFE_IMPORTACIONES
})
