<template>
  ...
  :previous-page-url="`/cargaconsolidada/documentacion/completados/pasos/${id}`">
      <template #actions>
        <UButton label="Fechas y Horarios" color="primary" variant="solid" class="py-3 hidden md:flex" icon="i-heroicons-calendar"
          @click="navigateTo(`/cargaconsolidada/documentacion/completados/entrega/fechas-horarios/${id}`)" />
      </template>
      <template #body-top>
  ...
  },
  {
    id: 'accion', header: 'Accion', cell: ({ row }) => h('div', { class: 'flex gap-1 items-center flex-wrap' }, [
      h(UButton, {
        ...
        onClick: () => navigateTo(`/cargaconsolidada/documentacion/completados/entrega/firma-carga/${row.original.id_cotizacion || row.original.id}?id_contenedor=${id}`)
      }),
...
const goToClienteDetalle = (data: any) => {
  ...
  navigateTo(`/cargaconsolidada/documentacion/completados/entrega/clientes/${cid}`)
}
