<template>
    <div class="">
        <DataTable v-if="tab === 'prospectos'" title="" icon="" :data="cotizaciones" :columns="getProespectosColumns()"
            ...
            :previous-page-url="(currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES || currentId == ID_JEFEVENTAS || currentRole == ROLES.ADMINISTRACION) ? `/cargaconsolidada/coordinacion/abiertos/pasos/${id}` : `/cargaconsolidada/coordinacion/abiertos`"
            :show-body-top="true">
            <template #body-top>
        ...
        cell: ({ row }: { row: any }) => {
            return h('div', { class: 'flex gap-2' }, [
                ...
                h(UButton, {
                    ...
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizaciones/documentacion/${row.original.id}`)
                    }
                }),
                h(UButton, {
                    icon: 'i-heroicons-trash',
    ...
    },
    {
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h('div', { class: 'py-2' }, [
                ...
                cod_cotizacion ? h('div', { class: 'text-sm text-gray-500' }, `Cotización: ${cod_cotizacion}`) : null,
                (currentRole.value === ROLES.COORDINACION || (currentRole.value === ROLES.JEFE_IMPORTACIONES && route.path.includes('coordinacion')))
                    ? renderEstadoPermisoPorTipo(row.original.estado_permiso_por_tipo ?? [], row.original.id_tramite)
                    : null
            ].filter(Boolean))
        }
            ...
            return h('div', {
                ...
                return h('div', {
                    ...
                    h(UButton, {
                        ...
                        size: 'md',
                        onClick: () => {
                            navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizaciones/proveedor/documentacion/${proveedor.id}`)
                        }
                    }),
                    h(UButton, {
    ...
    if (newVal && newVal !== '') {
        try {
            ...
            if (newVal === 'prospectos') {
                navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizaciones/${id}?tab=prospectos${idCotizacionQuery}`)
                ...
                navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizaciones/${id}?tab=embarque${idCotizacionQuery}`)
                try { search.value = '' } catch (e) { /* ignore */ }
                await getCotizacionProveedor(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizaciones/${id}?tab=pagos${idCotizacionQuery}`)
