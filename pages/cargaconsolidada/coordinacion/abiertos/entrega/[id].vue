<template>
  ...
  :previous-page-url="`/cargaconsolidada/coordinacion/abiertos/pasos/${id}`">
      <template #actions>
        <UButton label="Fechas y Horarios" color="primary" variant="solid" class="py-3 hidden md:flex" icon="i-heroicons-calendar"
          @click="navigateTo(`/cargaconsolidada/coordinacion/abiertos/entrega/fechas-horarios/${id}`)" />
      </template>
      <template #body-top>
...
const goToClienteDetalle = (data: any) => {
  ...
  navigateTo(`/cargaconsolidada/coordinacion/abiertos/entrega/clientes/${cid}`)
}
