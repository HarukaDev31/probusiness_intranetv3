  ...
  <div class="md:p-6">
    ...
    <div class="flex flex-col md:flex-row">
      <PageHeader title="" subtitle="" icon="" :hide-back-button="false" class="w-full"
        @back="navigateTo(`/cargaconsolidada/coordinacion/abiertos/pasos/${id}`)" />
...
const goBack = () => {
  navigateTo(`/cargaconsolidada/coordinacion/abiertos/pasos/${id}`)
}
...
watch(activeTab, async (newVal, oldVal) => {
  ...
  if (newVal === 'general') {
    navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizacion-final/${id}?tab=general`)
    await getGeneral(Number(id))
  }
  if (newVal === 'pagos') {
    navigateTo(`/cargaconsolidada/coordinacion/abiertos/cotizacion-final/${id}?tab=pagos`)
    await getPagos(Number(id))
  }
  await getHeaders(Number(id))
