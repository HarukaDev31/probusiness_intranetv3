<template>
    <div class="">
        <DataTable v-if="tab === 'prospectos'" title="" icon="" :data="cotizaciones" :columns="getProespectosColumns()"
            ...
            :previous-page-url="(currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES || currentId == ID_JEFEVENTAS || currentRole == ROLES.ADMINISTRACION) ? `/cargaconsolidada/coordinacion/completados/pasos/${id}` : `/cargaconsolidada/coordinacion/completados`"
            :show-body-top="true">
            <template #body-top>
        ...
        cell: ({ row }: { row: any }) => {
            return h('div', { class: 'flex gap-2' }, [
                ...
                h(UButton, {
                    ...
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/documentacion/${row.original.id}?soloVista=1`)
                    }
                }),
                h(UButton, {
                    icon: 'i-heroicons-trash',
        ...
        cell: ({ row }: { row: any }) => {
            return h('div', {
                ...
                h(UButton, {
                    ...
                    title: 'Ver documentación cotizadores',
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/documentacion/${row.original.id}`)
                    }
                }),
                h(UButton, {
                    icon: 'i-heroicons-arrow-right',
            ...
            return h('div', {
                ...
                return h('div', {
                    ...
                    h(UButton, {
                        ...
                        size: 'md',
                        onClick: () => {
                            navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/proveedor/documentacion/${proveedor.id}`)
                        }
                    }),
                    h(UButton, {
    ...
    if (newVal && newVal !== '') {
        try {
            ...
            if (newVal === 'prospectos') {
                navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/${id}?tab=prospectos${idCotizacionQuery}`)
                ...
                navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/${id}?tab=embarque${idCotizacionQuery}`)
                try { search.value = '' } catch (e) { /* ignore */ }
                await getCotizacionProveedor(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/coordinacion/completados/cotizaciones/${id}?tab=pagos${idCotizacionQuery}`)
