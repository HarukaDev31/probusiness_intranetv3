  ...
  <div class="md:p-6">
    ...
    <div class="flex flex-col md:flex-row">
      <PageHeader title="" subtitle="" icon="" :hide-back-button="false" class="w-full"
        @back="navigateTo(`/cargaconsolidada/coordinacion/completados/pasos/${id}`)" />
      <div class=" hidden md:flex items-center gap-3 flex-wrap md:mb-4 w-full md:justify-end"
        v-if="currentRole === ROLES.COORDINACION || (currentRole === ROLES.CONTABILIDAD || currentRole === ROLES.ADMINISTRACION) || currentRole === ROLES.JEFE_IMPORTACIONES">
...
const goBack = () => {
  navigateTo(`/cargaconsolidada/coordinacion/completados/pasos/${id}`)
}
...
watch(activeTab, async (newVal, oldVal) => {
  ...
  if (newVal === 'general') {
    navigateTo(`/cargaconsolidada/coordinacion/completados/cotizacion-final/${id}?tab=general`)
    await getGeneral(Number(id))
  }
  if (newVal === 'pagos') {
    navigateTo(`/cargaconsolidada/coordinacion/completados/cotizacion-final/${id}?tab=pagos`)
    await getPagos(Number(id))
  }
  await getHeaders(Number(id))
