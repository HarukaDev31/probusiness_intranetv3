    ...
    <template>
        <div class="p-0 md:p-6">
            <DataTable v-if="tab === 'general'" title="" icon="" :data="clientes" :columns="getColumnsGeneral()"
                ...
                :previous-page-url="(currentRole == ROLES.COORDINACION || currentId == ID_JEFEVENTAS || currentRole === ROLES.DOCUMENTACION || currentRole === ROLES.JEFE_IMPORTACIONES || currentRole == ROLES.ADMINISTRACION) ? `/cargaconsolidada/coordinacion/completados/pasos/${id}` : `/cargaconsolidada/coordinacion/completados`">
                <template #body-top>
            ...
            <DataTable v-if="tab === 'variacion'" title="" icon="" :data="clientesVariacion" :columns="columnsVariacion"
                ...
                :hide-back-button="false"
                :show-pagination="false" @export="exportData"
                :previous-page-url="(currentRole == ROLES.COORDINACION || currentId == ID_JEFEVENTAS || currentRole === ROLES.DOCUMENTACION || currentRole === ROLES.JEFE_IMPORTACIONES) ? `/cargaconsolidada/coordinacion/completados/pasos/${id}` : `/cargaconsolidada/coordinacion/completados`"
                empty-state-message="No se encontraron registros de clientes."
            ...
            <DataTable v-if="tab === 'pagos'" title="" icon="" :data="clientesPagos" :columns="columnsPagos"
                ...
                :show-body-top="true"
                :show-pagination="false" @export="exportData"
                :previous-page-url="(currentRole == ROLES.COORDINACION || currentRole == ROLES.JEFE_IMPORTACIONES || currentId == ID_JEFEVENTAS) ? `/cargaconsolidada/coordinacion/completados/pasos/${id}` : `/cargaconsolidada/coordinacion/completados`"
                empty-state-message="No se encontraron registros de clientes."
...

    {
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h(UButton, {
                ...
                size: 'xs',
                onClick: () => {
                    navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/documentacion/${row.original.id_cotizacion}`)
                }
            },
            )
        }
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h('div', { class: 'flex items-center gap-1' }, [
                ...
                h(UButton, {
                    ...
                    size: 'xs',
                    onClick: () => {
                        navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/documentacion/${row.original.id}`)
                    }
                })
            ])
        }
...
watch(() => tab.value, async (newVal) => {
    if (newVal && newVal !== '') {
        try {
            if (newVal === 'general') {
                navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/${id}?tab=general`)
                ...
                navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/${id}?tab=embarcados`)
                try { searchEmbarcados.value = '' } catch (e) { /* ignore */ }
                await getEmbarcados(Number(id))
            } else if (newVal === 'variacion') {
                navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/${id}?tab=variacion`)
                try { searchVariacion.value = '' } catch (e) { /* ignore */ }
                await getClientesVariacion(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`/cargaconsolidada/coordinacion/completados/clientes/${id}?tab=pagos`)
