<template>
  ...
  :previous-page-url="`/cargaconsolidada/coordinacion/completados/pasos/${id}`">
      <template #actions>
        <UButton label="Fechas y Horarios" color="primary" variant="solid" class="py-3 hidden md:flex" icon="i-heroicons-calendar"
          @click="navigateTo(`/cargaconsolidada/coordinacion/completados/entrega/fechas-horarios/${id}`)" />
      </template>
      <template #body-top>
...
// Columnas actualizadas para Entregas (completados) – mismas que en 'completados'
  ...
  },
  {
    id: 'accion', header: 'Accion', cell: ({ row }) => h('div', { class: 'flex gap-1 items-center flex-wrap' }, [
      h(UButton, {
        ...
        onClick: () => { void navigateTo(`/cargaconsolidada/coordinacion/completados/entrega/firma-carga/${row.original.id_cotizacion || row.original.id}?id_contenedor=${id}`) }
      }),
...
const goToClienteDetalle = (data: any) => {
  ...
  navigateTo(`/cargaconsolidada/coordinacion/completados/entrega/clientes/${cid}`)
}
  ...
  { accessorKey: 'pagado', header: 'Pagado', cell: ({ row }) => formatCurrency(row.original.pagado, 'PEN') },
  {
    id: 'adelantos', header: 'Adelantos', cell: ({ row }) => {
      ...
      return !row.original.id_contenedor_pago ? h(PagoGrid as any, {
        ...
        currency: 'PEN',
        numberOfPagos: (currentRole.value === ROLES.COORDINACION || currentRole.value === ROLES.JEFE_IMPORTACIONES) ? 3 : pagosDetails.length,
