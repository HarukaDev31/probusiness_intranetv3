  ...
  <div class="md:p-6 p-0">
    ...
    <template v-if="loading">
      ...
      <UCard>
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-7 gap-4 pb-2 border-b">
            <USkeleton v-for="i in 7" :key="i" class="h-4 w-full" />
          </div>
          
          <!-- Filas de tabla skeleton -->
          <div v-for="row in 5" :key="row" class="grid grid-cols-7 gap-4 py-3">
            <USkeleton v-for="col in 7" :key="col" class="h-4 w-full" />
          </div>
        </div>
      </UCard>
    </template>
    ...
    <template v-else>
      <DataTable 
        ...
        subtitle="Transacciones detalladas por contenedor"
        ...
        :primary-search-placeholder="'Buscar por contenedor...'" 
        :show-filters="false" 
        :show-export="true"
...
// Columnas de la tabla (sin columna de vendedor)
