...
import type { FacturaGuiaViewProps } from './types'
...
const props = withDefaults(defineProps<FacturaGuiaViewProps>(), {
      ...
      return cellWrap('')(h('div', { class: 'flex items-center gap-1 flex-wrap' }, [
        h(UTooltip, { text: 'Ver detalle contabilidad', placement: 'top' }, {
          default: () => h(UButton, {
            ...
            size: 'sm',
            onClick: () => {
              navigateTo(`${basePath.value}/factura-guia/clientes/${row.original.id_cotizacion}?carga=${carga.value || ''}`)
            }
          })
        }),
        h(UTooltip, { text: 'Ver formulario comprobante', placement: 'top' }, {
          default: () => h(UButton, {
            ...
            size: 'sm',
            onClick: () => {
              navigateTo(`${basePath.value}/factura-guia/formulario-comprobante/${row.original.id_cotizacion}`)
            }
          })
        }),
