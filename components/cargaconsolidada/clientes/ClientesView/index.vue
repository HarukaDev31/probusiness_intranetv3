...
const completeUrl = (url: string): string => {
    ...
    return BACKEND_FILES_BASE_URL + url
}
        ...
        cell: ({ row }: { row: any }) => {
            ...
            return h('div', { class: 'flex items-center gap-1' }, [
                ...
                h(UButton, {
                    ...
                    size: 'xs',
                    onClick: () => {
                        navigateTo(`${basePath.value}/clientes/documentacion/${row.original.id}`)
                    }
                })
            ])
        }
...
watch(() => tab.value, async (newVal) => {
    if (newVal && newVal !== '') {
        try {
            if (newVal === 'general') {
                navigateTo(`${basePath.value}/clientes/${id}?tab=general`)
                ...
                navigateTo(`${basePath.value}/clientes/${id}?tab=embarcados`)
                try { searchEmbarcados.value = '' } catch (e) { /* ignore */ }
                await getEmbarcados(Number(id))
            } else if (newVal === 'variacion') {
                navigateTo(`${basePath.value}/clientes/${id}?tab=variacion`)
                try { searchVariacion.value = '' } catch (e) { /* ignore */ }
                await getClientesVariacion(Number(id))
            } else if (newVal === 'pagos') {
                navigateTo(`${basePath.value}/clientes/${id}?tab=pagos`)
