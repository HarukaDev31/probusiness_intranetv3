...
import type { DocumentoTipo, ModalAccionesProps } from './types'
import { ACTION_OPTIONS, CATEGORIAS_OPTIONS, DOCUMENTOS_OPTIONS } from './constants'
const { withSpinner } = useSpinner()
const { getProveedoresItems, solicitarDocumentos, getProveedoresPendingDocuments, enviarRecordatorios } = useGeneral()
const { showSuccess, showError } = useModal()
const props = defineProps<ModalAccionesProps>()
...
const selectedDocsByProveedor = ref<Record<number, DocumentoTipo[]>>({})
// Selección general (para todos los proveedores)
const selectedDocsGeneral = ref<DocumentoTipo[]>([])
...
const isDocSelectedGeneral = (doc: DocumentoTipo) => selectedDocsGeneral.value.includes(doc)
const toggleDocGeneral = (doc: DocumentoTipo, checked: boolean) => {
  if (checked) {
...
const isDocSelectedForCurrent = (doc: DocumentoTipo) => {
...
const toggleDocForCurrent = (doc: DocumentoTipo, checked: boolean) => {
...
const onProviderDocToggle = (doc: DocumentoTipo, checked: boolean) => {
