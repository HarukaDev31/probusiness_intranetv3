...
const props = withDefaults(defineProps<CotizacionesDocumentacionViewProps>(), {
  role: undefined
})
const currentRole = computed(() => props.role || authCurrentRole.value)
const basePath = computed(() => props.basePath)
const soloVista = computed(() => currentRole.value === ROLES.COORDINACION)
...
function navigateBack() {
  ...
  if (idContenedor) {
    navigateTo(`${basePath.value}/cotizaciones/${idContenedor}`)
  } else {
    navigateTo(`${basePath.value}/cotizaciones`)
  }
}
