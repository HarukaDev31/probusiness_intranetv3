<template>
  <div class="">
    ...
    <!-- Mobile list view -->
    <div v-if="role !== ROLES.DOCUMENTACION && role !== ROLES.FINANZAS" class="md:hidden mt-4">
...
import { ID_ORGANIZACION_ADMIN, ROLES, esOrganizacionSocio, roleEsComoJefeImportacion } from '~/constants/roles'
...
import {
  ...
  DEFAULT_BASE_PATH,
  ESTADO_LABEL_MAP_ALMACEN,
  ESTADO_LABEL_MAP_DEFAULT,
  FINANZAS_STATUS_OPTIONS,
  GINO_USER_ID,
} from './constants'

const props = withDefaults(
  defineProps<CargaConsolidadaCompletadosProps>(),
  { backRoute: null, basePath: DEFAULT_BASE_PATH }
)
...
const handleCreateConsolidado = async (data: ConsolidadoFormData) => {
  try {
    const payload = {
      id: data.id,
      carga: data.carga,
...
const columns: TableColumn<any>[] = [
  ...
  },
  {
    ...
    header: 'Acciones',
    cell: ({ row }) => {
      const canDelete = String(row.original.estado_china || '').toUpperCase() === 'PENDIENTE'
      const actions = [
        h(UButton, {
          size: 'xs',
...
const getEstadoLabel = (estado: string) => {
  ...
  if (isAlmacen.value) {
    return ESTADO_LABEL_MAP_ALMACEN[estado] || estado
  }
  return ESTADO_LABEL_MAP_DEFAULT[estado] || estado
}
