  ...
  <div class="md:p-6">
    ...
    <div class="flex flex-col md:flex-row">
      <PageHeader title="" subtitle="" icon="" :hide-back-button="false" class="w-full"
        @back="navigateTo(`${backBasePath}/pasos/${id}`)" />
...
const props = withDefaults(defineProps<{ role?: string, basePath: string, backBasePath?: string }>(), { backBasePath: undefined })
...
const generalColumns = ref<TableColumn<any>[]>([
...

  {
    ...
    cell: ({ row }: { row: any }) => {
      ...
      if (!val) return '—'
      return h(UBadge, { label: val, color: val === 'Lima' ? 'success' : 'warning', variant: 'subtle' })
    }
  },
...
const getPagosColumns = (): TableColumn<any>[] => {
  ...
  if (isContabilidad) {
    base.push(
      ...
      { accessorKey: 'concepto', header: 'Concepto', cell: () => 'Logística + impuestos' }
    )
  }
  base.push(
    { accessorKey: 'total_logistica_impuestos', header: 'Importe', cell: ({ row }: { row: any }) => formatCurrency(row.original.total_logistica_impuestos) },
    { accessorKey: 'pagado', header: 'Pagado', cell: ({ row }: { row: any }) => formatCurrency(row.original.total_pagos) }
  )
  if (isContabilidad) {
    base.push({
    ...
    },
    {
      ...
      cell: ({ row }: { row: any }) => h(UTooltip, { text: 'Ver documentación', placement: 'top' }, {
        default: () => h(UButton, {
          ...
          onClick: () => {
            const target = `${basePath.value}/cotizacion-final/documentacion/${row.original.id_cotizacion}`
            const returnUrl = encodeURIComponent(`${basePath.value}/cotizacion-final/${id}?tab=cargos-extra`)
...
watch(activeTab, async (newVal, oldVal) => {
  ...
  if (newVal === 'general') {
    navigateTo(`${basePath.value}/cotizacion-final/${id}?tab=general`)
    await getGeneral(Number(id))
  }
  if (newVal === 'pagos') {
    navigateTo(`${basePath.value}/cotizacion-final/${id}?tab=pagos`)
    await getPagos(Number(id))
  }
  if (newVal === 'cargos-extra') {
    navigateTo(`${basePath.value}/cotizacion-final/${id}?tab=cargos-extra`)
