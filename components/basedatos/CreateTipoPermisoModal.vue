  ...
  <UModal :open="open" class="w-full max-w-md" :close="{ onClick: handleClose }" @update:open="(v: boolean) => { if (!v) handleClose() }">
    <template #header>
      <div class="flex items-center gap-2">
        ...
        <div>
          <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Nuevo tipo de permiso</h3>
          <p class="text-sm text-gray-500 dark:text-gray-400">Crear un tipo de permiso para el catálogo</p>
        </div>
      </div>
    </template>
