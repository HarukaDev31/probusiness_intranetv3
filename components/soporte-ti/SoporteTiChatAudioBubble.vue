<template>
  <div
    class="soporte-audio-bubble w-[min(100%,280px)] min-w-[220px] overflow-hidden rounded-lg shadow-sm ring-1"
    :class="bubbleClass"
  >
      ...
      <div class="relative shrink-0">
        ...
        <template v-else>
          <div
            ...
            :class="inverted ? 'bg-[#0b3d32] text-white' : 'bg-elevated text-primary'"
          >
            <UIcon name="i-heroicons-microphone" class="size-5" />
          </div>
        </template>
      </div>
      ...
      <div class="min-w-0 flex-1">
        <div class="flex items-center gap-1.5">
          ...
          >
            ...
            >
              <span
                class="size-3 rounded-full shadow"
                :class="inverted ? 'bg-[#4a7c6f] dark:bg-[#b8e0d4]' : 'bg-[#8696a0] dark:bg-[#aebac1]'"
              />
            </span>
          </div>
        </div>
        <div class="mt-0.5 flex items-center justify-between px-0.5">
          <span class="text-[11px] tabular-nums" :class="metaClass">{{ displayTime }}</span>
          <span v-if="timeLabel" class="text-[10px]" :class="metaClass">{{ timeLabel }}</span>
        </div>
      </div>
    </div>
...
const STOP_EVENT = 'soporte-ti-audio-stop'
...
const playerId = `soporte-audio-${Math.random().toString(36).slice(2, 10)}`

let rafId: number | null = null
let activePointerId: number | null = null

const waveformBars = computed(() => generateWaveformBars(props.url || 'placeholder', 40))
...
const iconClass = computed(() =>
  props.inverted ? 'text-[#111b21] dark:text-white' : 'text-[#54656f] dark:text-white/90'
)
const speedBtnClass = computed(() => iconClass.value)
...
function onExternalStop(e: Event) {
  ...
  if (detail?.except === playerId) return
  const el = audioRef.value
  if (el && !el.paused) {
    el.pause()
    playing.value = false
...
async function togglePlay() {
  ...
  window.dispatchEvent(new CustomEvent(STOP_EVENT, { detail: { except: playerId } }))
  applyPlaybackRate()
  try {
    await el.play()
    playing.value = true
...
onMounted(() => {
  window.addEventListener(STOP_EVENT, onExternalStop)
})

onBeforeUnmount(() => {
  window.removeEventListener(STOP_EVENT, onExternalStop)
  endDrag()
  stopPlayback()
})
</script>
