<template>
  <aside class="flex h-full min-h-0 max-h-full flex-col overflow-hidden border-r border-default bg-white dark:bg-gray-900">
    <div class="flex shrink-0 flex-col gap-2 border-b border-default px-3 py-2.5">
      ...
      <USelect
        v-if="advisorOptions.length"
        :model-value="advisorFilter"
        ...
        @update:model-value="emit('update:advisorFilter', $event)"
      />
      <UInput
        :model-value="search"
      ...
      >
        ...
        <div class="mt-1.5 flex items-center justify-between gap-1">
          <div class="flex min-w-0 items-center gap-1">
            <UBadge
              v-if="rowConversation(index)?.pipeline_stage_label"
              color="neutral"
              variant="outline"
              size="xs"
              class="max-w-[88px] truncate"
            >
              {{ rowConversation(index)?.pipeline_stage_label }}
            </UBadge>
            <UBadge
...
export type CopilotoAdvisorFilterOption = {
  label: string
  value: string
}
...
const props = withDefaults(
  defineProps<{
    ...
    loading?: boolean
    readonly?: boolean
    advisorOptions?: CopilotoAdvisorFilterOption[]
...
const emit = defineEmits<{
  ...
  'update:advisorFilter': [value: string]
  'new-contact': []
  sync: []
}>()
