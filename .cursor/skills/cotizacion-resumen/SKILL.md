...
- Si se **reemplaza el archivo** en COTIZADO: se borran proveedores e ítems **sin** `code_supplier` y se cargan los del documento nuevo. Los que ya tienen `code_supplier` se conservan (`id` + código).
...
- Rutas: `routes/modules/carga-consolidada.php` prefix `cotizacion-resumen`
  ...
  - `PUT /{id}` editar · `POST /{id}/duplicar` · `POST /{id}/partir`
...
- Listado: editar (solo COTIZADO), duplicar, eliminar, archivos, confirmar con contenedor. Encima de la tabla, mismos KPIs de Customers (CBM warehouse, clientes, códigos, NC) limitados a la org del usuario. Total NC filtra China = NC. Tarifa, FOB, ISD, logística e impuesto se guardan en BD (tarifa = logística / CBM en el back **solo si CBM ≥ 1**; si CBM < 1, p. ej. 0.99, tarifa = logística tal cual) y el listado solo pinta lo persistido. La logística se toma de **costos**: `Servicio de importación` o cualquier concepto que diga logística (internacional, marítima, etc.). ISD se clasifica si el documento trae el concepto (`ISD` o “salida de divisa”) y no entra en logística. Flete, transferencia y seguro no entran en logística.
- Wizard: el PDF/Excel se lee con IA (Excel se convierte a texto). Costos extraídos se mapean; si solo vienen totales se crean líneas Logística / FOB / Impuestos / ISD.
- Prospectos y Embarcados (org ≠ 1) comparten headers: bandera + `CBM:` (China y destino desde `pais_flags`), CBM Pendiente, CBM IMO, Fob, Total ISD, Logística, Impuestos. Sin nombre de país. La bandera de destino no se hardcodea a Perú: sale de `id_pais` del consolidado.
- Prospectos (org ≠ 1): Nº, Fecha, Contacto, T.Cliente, Volumen, Fob, ISD, Logística, Impuestos, Tarifa, Descuento, Cotización, Estado, Acciones. Sin Qty Item ni Cargos extra. El ojo de documentación en Acciones solo si está CONFIRMADO.
- Embarcados (org ≠ 1): Asesor, Status China, N, Contacto, T.Rotulado (PENDIENTE/GENERAL). Pasar a GENERAL no guarda: abre el modal de enviar rotulado ( Pendiente + Rotulado mostrado como General). Productos, F. Llegada, Qty box, CBM total, Weight, Supplier, Code supplier (solo lectura), Supplier phone, bloque China (span de solo lectura: Status, QTY, CBM, peso y Arrive Date; F. Llegada también si viene de China), Acciones (ojo, guardar, hamburguesa: enviar rotulado con las mismas dos opciones; Enviar solo si al menos uno no está en PENDIENTE; Reenviar si ya se envió; pedir documentos, recordatorio, enviar inspección; **Partir cotización** si hay 2+ proveedores: mueve un subconjunto a otro consolidado de la misma org/país, sin pagos ni Excel Drive). Sin Rotulo status. `tipo_rotulado` enum incluye `pendiente` (default). WhatsApp rotulado: conexión general actual; EMTA por org queda pendiente.
- Clientes (org ≠ 1, `/cargaconsolidada/abiertos/clientes/{id}`): solo cotizaciones **CONFIRMADO** (no COTIZADO). Solo tabs Documentación y Variación. Documentación mezcla seguimiento + docs: Nº, Contacto, T.Cliente, Volumen, Fob, ISD, Logística, Impuesto, Tarifa, Productos, Code Supplier, Invoice, Packing list, Excel Conf., Acciones (hamburguesa: pedir documentos y recordatorios; ojo: misma vista de documentación con tabs Documentación y Excel confirmación, sin Volumen documento ni Valor documento). Variación: Nº, Asesor, Contacto, T.Cliente, Tarifa, Vol. Cot, Vol. China, Variación (solo Cot vs China; sin Valor Cot, Valor Doc ni Vol. Doc). Headers de ambos tabs: bandera + `CBM:` (China y país del contenedor), Qty clientes, Qty items, Fob, Total ISD, Logística, Impuestos.
