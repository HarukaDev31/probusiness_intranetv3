import type { WaCopilotoMessage } from '~/types/wa-copiloto'
...
export function mergeWaCopilotoMessage(prev: WaCopilotoMessage, incoming: WaCopilotoMessage): WaCopilotoMessage {
...
export function sortMessagesChronologically(list: WaCopilotoMessage[]): WaCopilotoMessage[] {
...
export function mergeMessageLists(...sources: WaCopilotoMessage[][]): WaCopilotoMessage[] {
  const byId = new Map<number, WaCopilotoMessage>()
  for (const list of sources) {
    for (const m of list) {
      const id = waMessageNumericId(m.id)
      if (!id) continue
      const prev = byId.get(id)
      byId.set(id, prev ? mergeWaCopilotoMessage(prev, { ...m, id }) : { ...m, id })
    }
  }
